...
def send_verification_email(request: EmailRequest, db: Session = Depends(get_db), fastapi_request: Request = None):
    ...
    # 多语言处理
    lang = request.lang
    if not lang and fastapi_request is not None:
        lang = fastapi_request.headers.get('accept-language', 'yue').split(',')[0][:2]
    if lang not in ['zh', 'yue', 'en']:
        lang = 'yue'
    # Send verification email
    email_sent = email_service.send_verification_email(request.email, request.email.split('@')[0], verification_token, lang=lang)
